...
prefer). 0.11.0 is client-side only, but after upgrading from anything older
restart it so the 0.10.0 daemon-side fixes are served: the single-daemon
election on `<socket>.lock`, waits pinned to the occupant they started on,
per-server pane identity, and the earlier `pair` / `unpair`, `peer_send`,
`limited` sweep and merge-key features. Run
